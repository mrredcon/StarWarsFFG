...
export default class ImportHelpers {
  ...
  static findEntityByImportId(type, id) {
    return game.data[type].find((item) => {
      return item.flags.genesys.ffgimportid === id;
    });
  }
      ...
      if (!CONFIG.temporary[packid]) {
        ...
        if (pack.documentName === type && !pack.locked) {
          ...
          for (var i = 0; i < content.length; i++) {
            CONFIG.temporary[packid][content[i].data?.flags?.genesys?.ffgimportid] = duplicate(content[i]);
          }
        }
      } else {
    ...
    if (adversaryData.Weapons?.CharWeapon) {
      ...
      await this.asyncForEach(adversaryData.Weapons.CharWeapon, async (w) => {
        try {
          ...
          const weaponItems = adversary.items.filter((s) => s.flags.genesys.ffgimportid === weapon.flags.genesys.ffgimportid);
    
          if (weaponItems.length > 0) {
            for (let i = 0; i < adversary.items.length; i += 1) {
              if (adversary.items[i].type === "weapon" && adversary.items[i].flags.genesys.ffgimportid === weapon.flags.genesys.ffgimportid) {
    ...
    if(adversaryData.Talents?.CharTalent)
    {
      ...
      await this.asyncForEach(adversaryData.Talents.CharTalent, async (t) => {
        try{
          ...
          delete talent._id;
          const talentItems = adversary.items.filter((s) => s.flags.genesys.ffgimportid === talent.flags.genesys.ffgimportid);
          if (talentItems.length > 0) {
            for (let i = 0; i < adversary.items.length; i += 1) {
              if (adversary.items[i].type === "talent" && adversary.items[i].flags.genesys.ffgimportid === talent.flags.genesys.ffgimportid) {
      ...
      await this.asyncForEach(adversaryData.Armor.CharArmor, async (w) => {
        try {
          ...
          if(compArmor) {
            ...
            delete armor._id;
            const armorItems = adversary.items.filter((s) => s.flags.genesys.ffgimportid === armor.flags.genesys.ffgimportid);
      
            if (armorItems.length > 0) {
              for (let i = 0; i < adversary.items.length; i += 1) {
                if (adversary.items[i].type === "armor" && adversary.items[i].flags.genesys.ffgimportid === armor.flags.genesys.ffgimportid) {
      ...
      await this.asyncForEach(adversaryData.Gear.CharGear, async (w) => {
        try {
          ...
          if(compGear){
            ...
            let gearItem = adversary.items.find((s) => s.flags.genesys.ffgimportid === gear.flags.genesys.ffgimportid);
  ...
  static async extractAdversaryPowers(adversaryData, adversary)
  {
    ...
    await this.asyncForEach(basicPowers, async (power) => {
      ...
      if(comForceAbility)
      {
        ...
        let forceItem = adversary.items.find((s) => s.flags.genesys.ffgimportid === force.flags.genesys.ffgimportid);
        if (forceItem) {
  ...
  static async nemesisImport(adversaryData, updateDialog)
  {
    ...
    const exists = game.data.actors.find((actor) => actor.flags.genesys?.ffgimportid == npcKey);
  ...
  static async characterImport(data) {
    try {
      ...
      const exists = game.data.actors.find((actor) => actor.flags?.genesys?.ffgimportid === characterData.Character.Key);
      ...
      try {
        ...
        if (speciesDBItem) {
          ...
          let speciesItem = character.items.find((s) => s.flags.genesys.ffgimportid === species.flags.genesys.ffgimportid);
      ...
      try {
        ...
        if (career) {
          ...
          let careerItem = character.items.find((s) => s.flags.genesys.ffgimportid === career.flags.genesys.ffgimportid);
          ...
          if (characterData?.Character?.Specializations?.CharSpecialization?.length) {
            ...
            await this.asyncForEach(characterData.Character.Specializations.CharSpecialization, async (spec) => {
              if (spec.isStartingSpec && spec.isStartingSpec === "true") {
                ...
                let specializationItem = character.items.find((s) => s.flags.genesys.ffgimportid === specialization.flags.genesys.ffgimportid);
                ...
                try {
                  ...
                  let specializationItem = character.items.find((s) => s.flags.genesys.ffgimportid === newspec.flags.genesys.ffgimportid);
      ...
      await this.asyncForEach(forcepowers, async (power) => {
        try {
          ...
          let forceItem = character.items.find((s) => s.flags.genesys.ffgimportid === force.flags.genesys.ffgimportid);
      ...
      if (characterData.Character?.Weapons?.CharWeapon) {
        ...
        await this.asyncForEach(characterData.Character.Weapons.CharWeapon, async (w) => {
          try {
            ...
            const weaponItems = character.items.filter((s) => s.flags.genesys.ffgimportid === weapon.flags.genesys.ffgimportid);

            if (weaponItems.length > 0) {
              for (let i = 0; i < character.items.length; i += 1) {
                if (character.items[i].type === "weapon" && character.items[i].flags.genesys.ffgimportid === weapon.flags.genesys.ffgimportid) {
      ...
      if (characterData.Character?.Armor?.CharArmor) {
        ...
        await this.asyncForEach(characterData.Character.Armor.CharArmor, async (w) => {
          try {
            ...
            delete armor._id;
            const armorItems = character.items.filter((s) => s.flags.genesys.ffgimportid === armor.flags.genesys.ffgimportid);

            if (armorItems.length > 0) {
              for (let i = 0; i < character.items.length; i += 1) {
                if (character.items[i].type === "armor" && character.items[i].flags.genesys.ffgimportid === armor.flags.genesys.ffgimportid) {
      ...
      if (characterData.Character?.Gear?.CharGear) {
        ...
        await this.asyncForEach(characterData.Character.Gear.CharGear, async (w) => {
          try {
            ...
            let gearItem = character.items.find((s) => s.flags.genesys.ffgimportid === gear.flags.genesys.ffgimportid);
  ...
  static async addImportItemToCompendium(type, data, pack, removeFirst) {
    let entry = await ImportHelpers.findCompendiumEntityByImportId(type, data.flags.genesys.ffgimportid, pack.collection);
    let objClass;
    let dataType;
    switch (type) {
      case "Item": {
    ...
    if (!entry) {
      ...
      CONFIG.temporary[pack.collection][data.flags.genesys.ffgimportid] = duplicate(crt);
    } else {
      let upd;
      if (removeFirst) {
      ...
      CONFIG.temporary[pack.collection][data.flags.genesys.ffgimportid] = upd;
    }
  }
  ...
  static async processDieMod(mod) {
    ...
    await ImportHelpers.asyncForEach(mod.DieModifier, async (dieMod) => {
      if (!dieMod) {
        ...
        const skillTheme = await game.settings.get("genesys", "skilltheme");
        const allSkillsLists = JSON.parse(await game.settings.get("genesys", "arraySkillList"));
  ...
  static async getTemplate(type) {
    const response = await fetch("systems/genesys/template.json");
